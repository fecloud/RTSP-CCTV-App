...
/**
 * Turns [SharedCameraStream]'s H.264 + AAC output into fragmented MP4 (ISO/IEC 14496-12) for the
 * dashboard's MSE preview -- registered as a listener by `com.zektopic.cctvapp.mse.MseVideoBridge`.
 * No vendored library writes fragmented MP4, so this hand-rolls the small subset of ISOBMFF boxes
 * MSE needs: an `ftyp`+`moov` init segment (rebuilt on codec/resolution change, see
 * [onInitSegmentChanged]) followed by one `moof`+`mdat` fragment per video NAL or audio unit.
 *
 * One H.264 (`avc1`) track + one AAC-LC (`mp4a`) track, fixed track IDs, no B-frames, no editable
 * sample tables (every sample lives in a fragment, per `mvex`).
 *
 * The client uses `sourceBuffer.mode = 'segments'` and seeks to `sourceBuffer.buffered.start(0)`
 * once buffered (see dashboard.html) rather than trusting a server-computed offset -- `buffered`
 * on a multi-track `SourceBuffer` is the *intersection* of both tracks' ranges, so that's always
 * a point where both actually have data. A new viewer only ever receives fmp4 fragments starting
 * from a fresh keyframe (see `MseStreamSocket.enqueue`'s gating), so that intersection forms
 * cleanly instead of straddling a stale, disconnected one. This only works because [resetSession]
 * resets both tracks' clocks together once per real streaming session -- see [onVideoInfo]'s kdoc
 * for why that reset can't just happen on every SPS/PPS delivery.
 */
class Fmp4Fragmenter(
    private val onFragment: (fragment: ByteArray, kind: FragmentKind) -> Unit,
    /** Skips per-frame work when nobody's watching; never gates [onVideoInfo]/[onAudioFormat]. */
     ...
     * fragments until [VIDEO_KEYFRAME] resyncs it. [AUDIO] samples have no such dependency and
     * are never gated -- see `MseStreamSocket.enqueue`.
     */
    enum class FragmentKind { VIDEO_KEYFRAME, VIDEO_DELTA, AUDIO }
    ...
    /** Called by `MseVideoBridge.onStreamStarted` on each real `startStream()` call; resets both tracks' clocks together. */
    fun resetSession() {
        videoBaseUs = -1L
        lastVideoPtsUs = -1L
    ...
    /** Some encoders redeliver SPS/PPS mid-session (e.g. after [requestKeyframe]) -- only [resetSession] resets the timeline, not this. */
    ...
    override fun getVideoData(videoBuffer: ByteBuffer, info: MediaCodec.BufferInfo) {
        ...
        onFragment(fragment, if (isKey) FragmentKind.VIDEO_KEYFRAME else FragmentKind.VIDEO_DELTA)
    }
    ...
    override fun getAudioData(audioBuffer: ByteBuffer, info: MediaCodec.BufferInfo) {
        ...
        onFragment(fragment, FragmentKind.AUDIO)
    }

    private fun rebuildInitSegmentIfNeeded() {
    ...
    /** Everything a newly-connecting viewer needs, snapshotted together under [initLock] so it can't straddle a concurrent rebuild. */
    data class ViewerSnapshot(val codecs: String, val initSegment: ByteArray) {
        override fun equals(other: Any?): Boolean {
            if (this === other) return true
            if (javaClass != other?.javaClass) return false
            other as ViewerSnapshot
            return codecs == other.codecs && initSegment.contentEquals(other.initSegment)
        }

        override fun hashCode(): Int = 31 * codecs.hashCode() + initSegment.contentHashCode()
    }

    fun snapshotForNewViewer(): ViewerSnapshot? = synchronized(initLock) {
        val codecs = codecsString ?: return@synchronized null
        val init = initSegment ?: return@synchronized null
        ViewerSnapshot(codecs, init)
    }
