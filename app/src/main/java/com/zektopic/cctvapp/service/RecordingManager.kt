...
class RecordingManager(
    ...
    private val camera: () -> RtspServerStream?
) {
    companion object {
     ...
     * unconditionally right after constructing its `RtspServerStream`.
     */
    fun attachTo(cam: RtspServerStream) {
        controller?.let { return }
        if (cam.isStreaming) {
            // The one call site this ordering actually depends on (CctvServerService
            // constructing RtspServerStream) always calls this first -- if that's ever no
