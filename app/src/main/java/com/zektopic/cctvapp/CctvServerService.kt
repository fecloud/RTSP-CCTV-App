...
class CctvServerService : Service(), ConnectChecker, SurfaceHolder.Callback {
    ...
    private fun startGalleryRecordingIfNeeded() {
        ...
        if (rtspServerCamera.isRecording()) return
    ...
    private fun beginNewRecordingSegment() {
        mediaStoreExecutor.execute {
            ...
            onMain {
                if (!::rtspServerCamera.isInitialized || !rtspServerCamera.isStreaming || !recordToGalleryEnabled) {
                ...
                try {
                    ...
                    if (pfd != null && Build.VERSION.SDK_INT >= Build.VERSION_CODES.O) {
                        rtspServerCamera.startRecord(pfd.fileDescriptor, recordListener)
                    } else if (entry.path != null) {
                        rtspServerCamera.startRecord(entry.path, recordListener)
                    } else {
                    ...
                    recordRotationHandler.postDelayed(
                        { if (rtspServerCamera.isRecording()) rtspServerCamera.stopRecord() },
    ...
    private fun stopGalleryRecording() {
        ...
        if (::rtspServerCamera.isInitialized && rtspServerCamera.isRecording()) {
